...
import { equipeLabel } from "@/lib/situacao";
...
function Page() {
  ...
  useEffect(() => { if (!authorized) navigate({ to: "/login/$role", params: { role: "planejamento" } }); }, [authorized]);
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-7xl space-y-4 p-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <h1 className="text-xl font-bold">Painel — Planejamento</h1>
      ...
      {editing && <DemandaEditor demanda={editing} role="planejamento" onClose={() => setEditing(null)} onSaved={load} />}
    </div>
  );
}
